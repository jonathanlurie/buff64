...
echo -n "Enter the name of your module (buffer64) + [ENTER]: "
read moduleName
if [[ ! -z "${moduleName}" ]]; then
  find . -type f -not -path "*.git/*" -not -path "*node_modules*" -not -path "*starter.sh*" -exec sed -i "s/buffer64/${moduleName}/g" {} \;
fi
